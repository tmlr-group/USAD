...
def compute_projection_matrix(model, path, device, reduction_method, reduction_dim, n_perturb, std, calibration_data=None):
    ...
    if builtins.MODEL_ARCH is not None:
        cache_dir = os.path.join("./", builtins.MODEL_ARCH, "projection_matrix")
    else:
        cache_dir = "./projection_matrix"
    ...
    if "transfer" in path:
        ...
        raise ValueError(f"Unknown model architecture: {path}")
    ...
    cache_file = os.path.join(cache_dir, f"{dataset_name}_{model_arch}_{reduction_method}_{reduction_dim_str}_proj_{n_perturb}_{np.round(std, 4)}.pt")
...
def sigma_from_perturb(X_te, std, n_perturb, model, path, device, where=None, chunk_size=10,
    ...
    if builtins.MODEL_ARCH is not None:
        cache_dir = os.path.join("./", builtins.MODEL_ARCH, "Sigma_matrix")
    else:
...
def vd_u1(K, n, m):
    ...
    # K.fill_diagonal_(0)
    K = K - torch.eye(K.size(0)).to(K.device) * K.diag().sum() / K.size(0)
